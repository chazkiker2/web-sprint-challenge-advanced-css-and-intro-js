 ...
 */
function getArtistByIndex(arr: Artist[] | object, index: number) : string {
  const artist: Artist = arr[index];
...
function get20s(data: Artist[]): string[] {
  const a20s: string[] = data.map((artist: Artist): string => {
    const years: number = +artist.years.split(" - ");
 ...
 */
function removeArtist(arr: Artist[], index: number) : boolean {
...
function addArtist(info: Artist): Artist[] {
  artists.push(info);
  return artists;
}

const myInfo = {
  id: artists.length, //this is set to artists.length to make the id flexible & consistent with the array (regardless of previous function invocations)
  name: "Chaz",
  years: "2000 – current day",
  genre: "Web Design",
  bio: "lorem ipsum",
};
// console.log(addArtist(myInfo as Artist));
...
function lotsOfArt(arr: Artist[]): string[] {
  const artists100Plus: string[] = arr.map((artist) => {
...
function getHTML(data: Artist[]): void {
...
function randomize(arr: Artist[]): Artist[] {
  // FISHER-YATES SHUFFLE
  let currentIndex: number = arr.length;
  let tempVal: Artist, randomIndex: number;
